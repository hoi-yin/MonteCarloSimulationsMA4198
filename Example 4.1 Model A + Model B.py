...
def example1AB (S0, r , SD, T, n, k):
    if k <64:
        return example1ASCV(S0, r, SD, T, n, k)
    else:
        return example1ASCVIS(S0, r, SD, T, n, k)

import numpy as np
import matplotlib.pyplot as plt
k_values = np.linspace(40, 120, 201) # 201 evenly spaced numbers between 40 and 120

First = [example1AB (50, 0.05 , 0.2, 1, 10000, k) for k in k_values] # Model A + Model B
...
print(First[25]) #n=10000 K=50 Model A + Model B
print(First[50]) #n=10000 K=60 Model A + Model B
print(First[100]) #n=10000 K=80 Model A + Model B
print(First[150]) #n=10000 K=100 Model A + Model B
print(First[200]) #n=10000 K=120 Model A + Model B
...
y1 = list(map(lambda x:x[2],First)) # Model A + Model B
y2 = list(map(lambda x:x[2],Second)) # AS + CV b=b*
y3 = list(map(lambda x:x[2],Third)) # AS + CV b=b* + IS

plt.plot(k_values, y2, label='AS + CV',color='green')
plt.plot(k_values, y3, label='AS + CV + IS',color='orange')
plt.plot(k_values, y1, label='Model A + Model B',color='black')
...
y1 = list(map(lambda x:x[3],First)) # Model A + Model B
y2 = list(map(lambda x:x[3],Second)) # AS + CV b=b*
